...
export default ({ history }) => {
    ...
    React.useEffect(() => {
        if(!data) {
            const getDataPromise = makeCancelable(TvApi.get(authContext));
            getDataPromise
                .promise
                .then(info => {
                    if(info) {
        ...
        >
            ...
            >
                ...
                >
                    ...
                    >
                        { t("tvs.total", { total: data?.length || 0 }) }
                    </span>
                    <Link
                        to={{
                            pathname: "/tv/add"
                ...
                <Table
                    ...
                    headers={[
                        {
                            id: "id",
                            name: t("tvs.id_header"),
                            orderable: true,
                            filterable: true
                        },
                        {
                            id: "name",
                            name: t("tvs.name_header"),
                            orderable: true,
                            filterable: true
                        },
                        {
                            id: "published",
                            name: t("tvs.published_header"),
                            orderable: true,
                            filterable: true
                        }
                    ]}
                    ...
                    onRowClick={
                        index => {
                            history.replace("/tv", { tv: data[index] });
                        }
                    }
                />
            </div>
